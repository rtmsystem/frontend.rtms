...
const CustomInputHorizontal = (props: CustomInputHorizontalProps) => {
  ...
  // Vars
  const { meta, title, value, content, showExtraContent, extraContent } = data

  const renderData = () => {
    if (meta && title && content && showExtraContent) {
      return (
        <div className='flex flex-col bs-full is-full gap-1.5'>
          ...
          {typeof extraContent === 'string' ? <Content>{extraContent}</Content> : extraContent}
        </div>
      )
